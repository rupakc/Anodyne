...
async def test_create_encrypts_key_and_isolates_tenants(engine):  # type: ignore[no-untyped-def]
    reg = SqlVideoProviderRegistry(engine, FernetSecretStore(Fernet.generate_key()))
    t1, t2 = uuid4(), uuid4()
...
async def test_create_without_api_key_leaves_secret_ref_none(engine):  # type: ignore[no-untyped-def]
    reg = SqlVideoProviderRegistry(engine, FernetSecretStore(Fernet.generate_key()))
    tid = uuid4()
    cfg = await reg.create(
        tid,
    ...
    fetched = await reg.get(tid, cfg.id)
    assert fetched is not None
    assert fetched.secret_ref is None

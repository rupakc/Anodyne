"""`PyArrowExporter`: transcodes a stored `DatasetVersion` Parquet artifact to another format.
...
def _write_parquet(pf: pq.ParquetFile, batch_size: int) -> IO[bytes]:
    out = _spool()
...
def _write_arrow(pf: pq.ParquetFile, batch_size: int) -> IO[bytes]:
    out = _spool()
    writer = pa.ipc.new_stream(out, pf.schema_arrow)
    try:
        for batch in pf.iter_batches(batch_size=batch_size):
            writer.write_batch(batch)
    finally:
...
def _write_csv(pf: pq.ParquetFile, batch_size: int) -> IO[bytes]:
    out = _spool()
    writer = pcsv.CSVWriter(out, pf.schema_arrow)
    try:
        for batch in pf.iter_batches(batch_size=batch_size):
            writer.write_batch(batch)
    finally:
...
def _write_json(pf: pq.ParquetFile, batch_size: int) -> IO[bytes]:
...
class PyArrowExporter(Exporter):
    ...
    async def export(
        ...
        *,
        ...
        encoded = _WRITERS[resolved](pf, batch_size)
        try:

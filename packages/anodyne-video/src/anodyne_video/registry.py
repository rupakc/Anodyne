"""SQL-backed `VideoProviderRegistry`. Mirrors `anodyne_llm.registry.SqlModelRegistry`
exactly (same tenant-session + RLS + encrypted-secret-ref pattern), against the
...
from anodyne_storage.db import tenant_session, video_provider_configs
...
def _row_to_config(row: object) -> VideoProviderConfig:
...
class SqlVideoProviderRegistry(VideoProviderRegistry):
    ...
    async def create(
        ...
        *,
        ...
        async with tenant_session(self._engine, tenant_id) as s:
            await s.execute(
                insert(video_provider_configs).values(
                    id=cid,
                    tenant_id=tenant_id,
                    name=name,
                    provider=provider,
    ...
    async def get(self, tenant_id: UUID, config_id: UUID) -> VideoProviderConfig | None:
        # Explicit tenant_id filter is defense-in-depth alongside Postgres RLS
        # (see `SqlModelRegistry.get`'s identical comment).
        async with tenant_session(self._engine, tenant_id) as s:
            row = (
                await s.execute(
                    select(video_provider_configs).where(
                        video_provider_configs.c.id == config_id,
                        video_provider_configs.c.tenant_id == tenant_id,
                    )
                )
            ).first()
    ...
    async def delete(self, tenant_id: UUID, config_id: UUID) -> None:
        async with tenant_session(self._engine, tenant_id) as s:
            await s.execute(
                delete(video_provider_configs).where(
                    video_provider_configs.c.id == config_id,
                    video_provider_configs.c.tenant_id == tenant_id,
                )
            )

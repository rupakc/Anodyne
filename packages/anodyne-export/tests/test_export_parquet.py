...
async def test_parquet_export_round_trips_fixture_table() -> None:
    store = _FakeObjectStore()
    table = _fixture_table()
    version = await _stored_version(store, table)

    artifact = await PyArrowExporter().export(version, store, format="parquet")

    assert artifact.format == "parquet"
    parquet_bytes = await store.get(artifact.object_key)
